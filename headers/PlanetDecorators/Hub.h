...
#pragma once
#include <iostream>
#include "Planet/PlanetDecorator.h"

using std::cout;

class Hub : public PlanetDecorator {
public:
	Hub(AbstractPlanet *p) : PlanetDecorator(p) { } //take planet to be "decorated"

	void render(); //render method to output the decorator type
	float getMovementSpeed() { return PlanetDecorator::getMovementSpeed(); } //get modified movementSpeed
	~Hub() {}
};